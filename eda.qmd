...
# === Ultra-Safe Lightweight Data Load ===
# Only grab the essential columns
cols_needed = [
...
# ✅ Read only first 5,000 rows to keep memory small
df = pd.read_csv(
...
# --- Robust extraction for numeric values from messy text ---
print(f"Raw rows after load: {len(df)}")

def extract_number(val):
    """Extract first numeric value from strings like '$120K per year', '5 years', etc."""
    if pd.isna(val):
        return np.nan
...
# --- Diagnose row counts at each stage ---
print(f"Raw rows after load: {len(df)}")
...
# --- Relaxed filters: keep broader ranges ---
# Allow salaries as low as 1k, up to 600k
df = df[df["Average_Salary"].between(1000, 600000, inclusive="both")]

# Allow experience between 0 and 60 years
df = df[df["MIN_YEARS_EXPERIENCE"].between(0, 60, inclusive="both")]

# Drop remaining NaNs
df = df.dropna(subset=["Average_Salary", "MIN_YEARS_EXPERIENCE"])

print(f"Remaining after filters: {len(df)}")

# --- If still large, take a small random sample ---
if len(df) > 2000:
    df = df.sample(2000, random_state=42)

print(f"✅ Data loaded safely: {len(df)} rows, {df.memory_usage(deep=True).sum()/1e6:.1f} MB in memory")
```
